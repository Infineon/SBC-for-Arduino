...
void setup() {
   /* Init LEDs */
    ...
    /* light up green lead 1 sec as it would go directly to sleep otherwise */
    delay (1000);    
        ...
        if(((SWK_statusVal & SBC_SWK_STAT_SYNC_Msk) >> SBC_SWK_STAT_SYNC_Pos) == SBC_SYNC_VALID_FRAME_RECEIVED) {
          ...
          /* SBC protocol handler is in sync, enter sleep mode now, turn off LEDs */
            ...
            if(err.flippedBitsMask != 0) {
                ...
                /* this piece of code can be reached if the magic WUF frame is sent before the CAN-PN handler is in SYNC */
                /* SBC is in woken state then and CAN-PN config is not valid then anymore */
